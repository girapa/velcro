...
class TestListTemplate:
    ...
    def test_boolean_renders_check(self, client, bookmark):
        assert "<td>✓</td>" in client.get("/bookmarks/").content.decode()

    def test_view_method_column(self, client, bookmark):
        assert "<td>example.com</td>" in client.get("/bookmarks/").content.decode()

    def test_fk_cell_links_to_related_detail(self, client, bookmark, collection):
        html = client.get("/bookmarks/").content.decode()
        assert f'<a href="/collections/{collection.pk}/">Reading</a>' in html

    def test_row_action_links(self, client, bookmark):
        html = client.get("/bookmarks/").content.decode()
        assert f'href="/bookmarks/{bookmark.pk}/edit/"' in html
        assert f'href="/bookmarks/{bookmark.pk}/delete/"' in html

    def test_empty_state(self, client, db):
        assert "No bookmarks yet." in client.get("/bookmarks/").content.decode()
    ...
    def test_pagination_uses_querystring_tag(self, client, collection, db):
        ...
        html = client.get("/bookmarks/?favourite=x").content.decode()
    ...
    def test_create_link_present(self, client, db):
        assert 'href="/bookmarks/new/"' in client.get("/bookmarks/").content.decode()


class TestDetailTemplate:
    def test_fields_rendered_as_definition_list(self, client, bookmark):
        html = client.get(f"/bookmarks/{bookmark.pk}/").content.decode()
